...
export class Messages {
  async uploadMessage(data: any, context: functions.https.CallableContext) {
    ...
    return db.collection("messages").doc().set(message);
  }

  async sendNotificationForMessage(data: any, context: functions.https.CallableContext) {
    const title: string = data.title;
    const snippet: string = data.snippet;
    const receivers: string[] = data.receivers;
    const registrationTokens: string[] = [];
    for (const receiver of receivers) {
      const group: FirebaseFirestore.CollectionReference = await db.collection("groups").doc(receiver).collection("priviledge");
      ...
      for (const uid of priviledgedList) {
        const userMap: FirebaseFirestore.DocumentSnapshot = await db.collection("user").doc(uid).get();
        const test = userMap.get("devtoken");
        if (test !== undefined) {
          const devtokensObject: { [key: string]: any } = test;
          for (const token of Object.values(devtokensObject)) {
