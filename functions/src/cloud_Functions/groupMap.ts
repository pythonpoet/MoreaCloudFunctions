...
export class GroupMap {
  ...
  async getChildAndHisParentsDevTokens(childuserIDs: Array<string>): Promise<Array<string>> {
    ...
    for (let i = 0; i < childuserIDs.length; i++) {
      ...
      if (child.exists) {
        ...
        if ("devtoken" in childData) {
          if (childData.devtoken instanceof Object || childData.devtoken instanceof Object) {
            ...
            console.error(
              "devToken is malformed: " + JSON.stringify(childData.devtoken) + "(User: " + childUserID + " ). And has type: " + typeof childData.devtoken
            );
          }
  ...
  async getDeviceTokenFromChildParents(childMap: any): Promise<Array<string>> {
    ...
    const devTokenArray = new Array<string>();
      ...
      if (parent.exists) {
        ...
        if ("devtoken" in parentData) {
          if (parent.devtoken instanceof Object || parentData.devtoken instanceof Object) {
            ...
            console.error(
              "devToken is malformed: " + JSON.stringify(parentData.devtoken) + "(User: " + parentUserID + " ). And has type: " + typeof parentData.devtoken
            );
          }
  ...
  async createUserPriviledgeEntry(data: any, context: functions.https.CallableContext) {
    ...
    try {
      if (userPriviledgeEntry.validate()) await priviledgeRef.create(userPriviledgeEntry.pack());
    } catch (e) {
  ...
  async updateUserPriviledgeEntry(data: any, context: functions.https.CallableContext) {
    ...
    userPriviledgeEntry.create("Teilnehmer", "local", customInfo, displayName);
    try {
      if (userPriviledgeEntry.validate()) await priviledgeRef.update(userPriviledgeEntry.pack());
    } catch (e) {
      console.error("could not create PriviledgeEntry because of: ", e);
    }
  ...
  async makeLeiter(data: any, context: functions.https.CallableContext) {
    ...
    return db.runTransaction((t) => {
      return t
        .get(groupRef)
        .then((dSgroup) => {
          const groupData: any = dSgroup.data();
          let priviledge: any;
          if ("Priviledge" in groupData) {
            priviledge = groupData["Priviledge"];
            priviledge[userID] = { "DisplayName": displayName, "Priviledge": 3 };
          } else {
            priviledge = { userID: { "DisplayName": displayName, "Priviledge": 3 } };
