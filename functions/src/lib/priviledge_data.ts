import {
  ...
  groupMapPriviledgeEntryCustomInfo,
...
export class PriviledgeEntry {
  ...
  constructor(data: any | undefined) {
    if (data != undefined) {
      if (groupMapPriviledgeEntryLocation in data) this.roleLocation = data[groupMapPriviledgeEntryLocation];
      else console.warn("PriviledgeEntry needs a ", groupMapPriviledgeEntryLocation);
      if (groupMapPriviledgeEntryType in data) this.roleType = data[groupMapPriviledgeEntryType];
      else console.warn("PriviledgeEntry needs a ", groupMapPriviledgeEntryType);
      if (groupMapGroupJoinDate in data) this.groupJoinDate = data[groupMapGroupJoinDate];
      else console.warn("PriviledgeEntry needs a ", groupMapGroupJoinDate);
      if (groupMapDisplayName in data) this.displayName = data[groupMapDisplayName];
      else console.warn("PriviledgeEntry needs a ", groupMapDisplayName);
      if (groupMapPriviledgeEntryCustomInfo in data) this.customInfo = data[groupMapPriviledgeEntryCustomInfo];
