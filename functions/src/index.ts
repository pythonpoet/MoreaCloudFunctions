...
admin.initializeApp();
import { CloudTeleblitz } from "./cloud_Teleblitz";
export { CloudTeleblitz as CloudTeleblitz } from "./cloud_Teleblitz";
...
export const anmeldungTeleblitz = db.firestore.document("events/{eventID}/Anmeldungen/{anmeldeUID}").onWrite(async (change, context) => {
  const tlbz = new CloudTeleblitz();
...
export const deleteUser = db.firestore.document("user/{userID}").onDelete(async (snap, context) => {
  if (
...
export const pushNotificationOnTeleblitzCreate = db.firestore.document("groups/{groupID}").onWrite(async (change, context) => {
  const pushNotification = new PushNotificationByTeleblitzCreated();
  return pushNotification.groupLevelInit(change, context);
});
export const pushNotificationOnTeleblitzWrite = db.firestore.document("events/{groupID}").onWrite(async (change, context) => {
  const pushNotification = new PushNotificationByTeleblitzCreated();
...
export const uploadAndNotifyMessage = db.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const messages = new Messages();
  await messages.uploadMessage(data, context);
  return messages.sendNotificationForMessage(data, context);
});

export const deactivateDeviceNotification = db.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  const userMap = new UserMap();
  return userMap.deactivateDeviceNotification(data, context);
});

export const resetAnmeldungenTeleblitz = db.firestore.document(pathEvents + "/{eventID}").onUpdate(async (change, context) => {
