...
export class CloudTeleblitz {
  ...
  signUpTeleblitzHistory(newData: any, context: functions.EventContext): Promise<any> {
    ...
    const anmeldeHistoryRef = db.collection("events").doc(eventID).collection("AnmeldungHistory").doc(anmeldeUID);

    return db.runTransaction((t) => {
      return t
        .get(anmeldeHistoryRef)
        .then((docSnapshot) => {
          if (docSnapshot.exists) {
            ...
            for (let i = 0; i < anmeldeStamp.length; i++) {
              const oldDate: Date = anmeldeStamp[i].Timestamp.toDate();
              if (newDate < oldDate) {
                anmeldeStamp.splice(i, 0, newData);
                aenderung = true;
                break;
              }
            }
